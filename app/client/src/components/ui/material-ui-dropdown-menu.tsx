...
const useInternalRipple = (
  ...
  const endPressAnimation = async () => {
    ...
    if (
      ...
      await new Promise((r) =>
        setTimeout(r, MINIMUM_PRESS_MS - (animation.currentTime as number))
      );
    }
          ...
          {React.cloneElement(children as React.ReactElement<any>, {
            children: (
              <div
                ...
                className={cn(
                  "relative flex flex-1 items-center px-4",
                  inset && "pl-12"
                )}
              >
  ...
  React.useEffect(() => {
    if (isActive && pageNode) {
      const observer = new ResizeObserver((entries) => {
        setMenuHeight(
          (entries[0] as any).borderBoxSize?.[0]?.blockSize ?? entries[0].contentRect.height
        );
      });
